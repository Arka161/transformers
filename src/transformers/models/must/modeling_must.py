...
_CONFIG_FOR_DOC = "MustConfig"
_TOKENIZER_FOR_DOC = "MustTokenizer"
_CHECKPOINT_FOR_DOC = "must-small"
...
MUST_PRETRAINED_MODEL_ARCHIVE_LIST = [
    ...
    "must-3b",
    "must-11b",
    # See all must models at https://huggingface.co/models?filter=must
]
...
####################################################
def load_tf_weights_in_must(model, config, tf_checkpoint_path):
...
PARALLELIZE_DOCSTRING = r"""
    ...
    Args:
        device_map (:obj:`Dict[int, list]`, optional, defaults to None):
            ...
            have fewer attention modules mapped to it than other devices. For reference, the must models have the
    ...
    Example::
            ...
            # Here is an example of a device map on a machine with 4 GPUs using must-3b, which has a total of 24 attention modules:
            model = mustForConditionalGeneration.from_pretrained('must-3b')
            device_map = {0: [0, 1, 2],
...
DEPARALLELIZE_DOCSTRING = r"""
    ...
    Example::
        ...
        # On a 4 GPU machine with must-3b:
        model = MustForConditionalGeneration.from_pretrained('must-3b')
        device_map = {0: [0, 1, 2],
...
class MustLayerNorm(nn.Module):
    def __init__(self, hidden_size, eps=1e-6):
        """
        Construct a layernorm module in the Must style No bias and no subtraction of mean.
        """
        super().__init__()
...
class MustDenseReluDense(nn.Module):
...
class MustDenseGatedGeluDense(nn.Module):
    def __init__(self, config):
        super().__init__()
...
class MustRouterLayer(nn.Module):
    ...
    def forward(self, inputs: torch.Tensor):
        ...
        expert_capacity = int(self.config.capacity_factor * int(batch_size * seq_len) // self.config.n_experts)
...
class MustAttention(nn.Module):
    def __init__(self, config: MustConfig, has_relative_attention_bias=False):
        super().__init__()
...
class MustLayerSelfAttention(nn.Module):
    def __init__(self, config, has_relative_attention_bias=False):
        super().__init__()
        self.SelfAttention = MustAttention(config, has_relative_attention_bias=has_relative_attention_bias)
        self.layer_norm = MustLayerNorm(config.d_model, eps=config.layer_norm_epsilon)
        self.dropout = nn.Dropout(config.dropout_rate)
...
class MustLayerCrossAttention(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.EncDecAttention = MustAttention(config, has_relative_attention_bias=False)
        self.layer_norm = MustLayerNorm(config.d_model, eps=config.layer_norm_epsilon)
        self.dropout = nn.Dropout(config.dropout_rate)
...
class MustPreTrainedModel(PreTrainedModel):
    ...
    config_class = MustConfig
    load_tf_weights = load_tf_weights_in_must
...
class MustStack(MustPreTrainedModel):
...
MUST_START_DOCSTRING = r"""

    The Must model was proposed in `Exploring the Limits of Transfer Learning with a Unified Text-to-Text Transformer
    ...
    Parameters:
        config (:class:`~transformers.MustConfig`): Model configuration class with all the parameters of the model.
            Initializing with a config file does not load the weights associated with the model, only the
            configuration. Check out the :meth:`~transformers.PreTrainedModel.from_pretrained` method to load the model
            weights.
"""

MUST_INPUTS_DOCSTRING = r"""
    Args:
        input_ids (:obj:`torch.LongTensor` of shape :obj:`(batch_size, sequence_length)`):
            Indices of input sequence tokens in the vocabulary. Must is a model with relative position embeddings so you
            should be able to pad the inputs on both the right and the left.

            Indices can be obtained using :class:`~transformers.MustTokenizer`. See
            :meth:`transformers.PreTrainedTokenizer.encode` and :meth:`transformers.PreTrainedTokenizer.__call__` for
            detail.

            `What are input IDs? <../glossary.html#input-ids>`__

            To know more on how to prepare :obj:`input_ids` for pretraining take a look a `Must Training
            <./must.html#training>`__.
        ...
        decoder_input_ids (:obj:`torch.LongTensor` of shape :obj:`(batch_size, target_sequence_length)`, `optional`):
            ...
            Must uses the :obj:`pad_token_id` as the starting token for :obj:`decoder_input_ids` generation. If
            :obj:`past_key_values` is used, optionally only the last :obj:`decoder_input_ids` have to be input (see
            :obj:`past_key_values`).

            To know more on how to prepare :obj:`decoder_input_ids` for pretraining take a look at `Must Training
            <./must.html#training>`__.
...
MUST_ENCODER_INPUTS_DOCSTRING = r"""
    Args:
        input_ids (:obj:`torch.LongTensor` of shape :obj:`(batch_size, sequence_length)`):
            Indices of input sequence tokens in the vocabulary. Must is a model with relative position embeddings so you
            should be able to pad the inputs on both the right and the left.
...
@add_start_docstrings(
    "The bare Must Model transformer outputting raw hidden-states without any specific head on top.",
    MUST_START_DOCSTRING,
)
class MustModel(MustPreTrainedModel):
    ...
    def __init__(self, config: MustConfig):
        ...
        self.encoder = MustStack(encoder_config, self.shared)
    ...
    def forward(
        ...
        Example::
            ...
            >>> from transformers import MustTokenizer, MustModel

            >>> tokenizer = MustTokenizer.from_pretrained('must-small')
            >>> model = MustModel.from_pretrained('must-small')
...
class MustForConditionalGeneration(MustPreTrainedModel):
    ...
    def forward(
        ...
        Examples::
            ...
            >>> from transformers import MustTokenizer, MustForConditionalGeneration

            >>> tokenizer = MustTokenizer.from_pretrained('must-small')
            >>> model = MustForConditionalGeneration.from_pretrained('must-small')
...
@add_start_docstrings(
    "The bare Must Model transformer outputting encoder's raw hidden-states without any specific head on top.",
    MUST_START_DOCSTRING,
)
class MustEncoderModel(MustPreTrainedModel):
    ...
    def forward(
        ...
        Example::
            ...
            >>> from transformers import MustTokenizer, MustEncoderModel
            >>> tokenizer = MustTokenizer.from_pretrained('must-small')
            >>> model = MustEncoderModel.from_pretrained('must-small')
            >>> input_ids = tokenizer("Studies have been shown that owning a dog is good for you", return_tensors="pt").input_ids  # Batch size 1
